...
fn test_escape_sequences_only_in_double_quotes() {
    ...
    assert!(result.is_ok(), "Escape sequences must work in double-quoted scalars per RFC 5.7");
    
    if let Ok(docs) = result {
        if let Some(value) = docs[0]["text"].as_str() {
            assert!(value.contains('\n'), "\\n must be interpreted as newline in double quotes");
            assert!(value.contains('\t'), "\\t must be interpreted as tab in double quotes");
        }
    }
    ...
    assert!(result.is_ok(), "Single-quoted scalars must parse successfully per RFC 5.7");
    
    if let Ok(docs) = result {
        if let Some(value) = docs[0]["text"].as_str() {
            assert!(value.contains("\\n"), "\\n must be literal in single quotes, not escape");
            assert!(value.contains("\\t"), "\\t must be literal in single quotes, not escape");
        }
    }
    ...
    if result.is_ok() {
        if let Some(value) = result.unwrap()[0]["text"].as_str() {
...
"#;
    ...
    assert!(result.is_ok(), "Block scalars must parse successfully per RFC 5.7");
    
    if let Ok(docs) = result {
        if let Some(value) = docs[0]["text"].as_str() {
            assert!(value.contains("\\n"), "\\n must be literal in block scalars, not escape");
            assert!(value.contains("\\t"), "\\t must be literal in block scalars, not escape");
        }
    }
}
...
fn test_c_style_escape_sequences() {
    ...
    for (escape_seq, expected_char, desc) in c_escapes {
        ...
        assert!(result.is_ok(), 
            "C-style escape {} ({}) must work per RFC 5.7", escape_seq, desc);
            
        if let Ok(docs) = result {
            if let Some(value) = docs[0]["test"].as_str() {
                assert!(value.contains(expected_char), 
                    "Escape {} must produce character {:?} (U+{:02X})", 
                    escape_seq, expected_char, expected_char as u32);
            }
        }
    }
}
...
fn test_yaml_specific_escape_sequences() {
    ...
    for (escape_seq, expected_char, desc) in yaml_escapes {
        ...
        let result = YamlLoader::load_from_str(&yaml);
        assert!(result.is_ok(), 
            "YAML-specific escape {} ({}) must work per RFC 5.7", escape_seq, desc);
...
fn test_hexadecimal_escape_sequences() {
    ...
    for (escape_seq, expected_char) in hex_8bit_cases {
        ...
        let result = YamlLoader::load_from_str(&yaml);
        assert!(result.is_ok(), "8-bit hex escape {} must work per RFC 5.7", escape_seq);
        
        if let Ok(docs) = result {
            if let Some(value) = docs[0]["test"].as_str() {
                assert!(value.contains(expected_char), 
                    "Hex escape {} must produce '{}'", escape_seq, expected_char);
            }
        }
    }
    ...
    for (escape_seq, expected_char) in hex_16bit_cases {
        let yaml = format!(r#"test: "{}""#, escape_seq);
        let result = YamlLoader::load_from_str(&yaml);
        assert!(result.is_ok(), "16-bit hex escape {} must work per RFC 5.7", escape_seq);
    ...
    for (escape_seq, expected_char) in hex_32bit_cases {
        let yaml = format!(r#"test: "{}""#, escape_seq);
        let result = YamlLoader::load_from_str(&yaml);
        assert!(result.is_ok(), "32-bit hex escape {} must work per RFC 5.7", escape_seq);
...
"#;
    ...
    assert!(result.is_ok(), "All escape sequences must work together per RFC 5.7");
    
    if let Ok(docs) = result {
        if let Some(value) = docs[0]["escapes"].as_str() {
...
fn test_escapes_in_different_contexts() {
    ...
    assert!(result.is_ok(), "Escape sequences in flow collections must work per RFC 5.7");
    ...
    assert!(result.is_ok(), "Multiple escape sequences must work per RFC 5.7");
}
...
fn test_escape_edge_cases() {
    ...
    assert!(result.is_ok(), "Single escape in string must work per RFC 5.7");
    ...
    assert!(result.is_ok(), "Maximum Unicode escape must work per RFC 5.7");
}
