...
fn test_utf8_encoding_support() {
    ...
    if let Some(value_str) = docs[0]["key"].as_str() {
        assert!(value_str.contains("世界"), "UTF-8 Chinese characters must be preserved");
        assert!(value_str.contains("🌍"), "UTF-8 emoji must be preserved");  
        assert!(value_str.contains("Здравствуй"), "UTF-8 Cyrillic must be preserved");
    }
}
...
fn test_byte_order_mark_detection() {
    ...
    assert!(result.is_ok(), "Must support UTF-8 BOM detection per RFC 5.2");
    
    // Verify BOM is handled correctly (typically stripped)
    let docs = result.unwrap();
    assert_eq!(docs.len(), 1);
    assert!(docs[0]["key"].as_str().is_some(), "BOM should not interfere with parsing");
}
...
fn test_multi_document_encoding_consistency() {
    ...
    assert!(result.is_ok(), "Multi-document streams must maintain encoding consistency per RFC 5.2");
...
fn test_bom_at_document_start() {
    ...
    assert!(result.is_ok(), "BOM at start of first document must be supported per RFC 5.2");
...
fn test_bom_in_quoted_scalars() {
    ...
    assert!(result.is_ok(), "BOM inside quoted scalars must be allowed per RFC 5.2");
    
    let docs = result.unwrap();
    if let Some(value_str) = docs[0]["key"].as_str() {
        assert!(value_str.contains('\u{FEFF}'), "BOM must be preserved in quoted scalar content");
    }
}
...
fn test_encoding_transparency() {
    ...
    assert_eq!(result_utf8[0]["message"].as_str(), result_bom[0]["message"].as_str(), 
...
fn test_ascii_character_encoding_deduction() {
    ...
    for yaml in ascii_start_cases {
        let result = YamlLoader::load_from_str(yaml);
        assert!(result.is_ok(), 
            "ASCII-starting stream '{}' must be parsed correctly per RFC 5.2", yaml);
    }
}
...
fn test_bom_detection_table_patterns() {
    ...
    assert!(result.is_ok(), "UTF-8 BOM pattern must be recognized per RFC 5.2");
    
    // Test that content after BOM is parsed correctly
    let docs = result.unwrap();
    assert!(docs[0]["test"].as_str().is_some(), "Content after BOM must parse correctly");
}
...
"#;
    ...
    assert!(result.is_ok(), "Complex Unicode content must work consistently per RFC 5.2");
    
    let docs = result.unwrap();
    assert_eq!(docs.len(), 3, "All documents must parse with consistent encoding");
