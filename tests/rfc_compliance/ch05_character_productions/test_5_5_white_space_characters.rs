...
fn test_white_space_character_recognition() {
    ...
    assert!(result.is_ok(), "Must recognize space (x20) as whitespace per RFC 5.5");
    
    let docs = result.unwrap();
    assert_eq!(docs[0]["key1"].as_str(), Some("value1"), "Space separation must work");
    assert_eq!(docs[0]["key2"].as_str(), Some("value2"), "Multiple spaces must work");
    
    // Test tab (x09) as whitespace
    let yaml_with_tabs = "key1:\tvalue1\nkey2:\t\tvalue2";
    let result = YamlLoader::load_from_str(yaml_with_tabs);
    assert!(result.is_ok(), "Must recognize tab (x09) as whitespace per RFC 5.5");
    
    let docs = result.unwrap();
    assert_eq!(docs[0]["key1"].as_str(), Some("value1"), "Tab separation must work");
    assert_eq!(docs[0]["key2"].as_str(), Some("value2"), "Multiple tabs must work");
}
...
fn test_mixed_whitespace() {
    ...
    assert!(result.is_ok(), "Must handle mixed space/tab whitespace per RFC 5.5");
    
    let docs = result.unwrap();
    assert_eq!(docs[0]["key1"].as_str(), Some("value1"), "Mixed whitespace separation must work");
    assert_eq!(docs[0]["key2"].as_str(), Some("value2"), "Complex mixed whitespace must work");
}
...
fn test_only_space_and_tab_are_whitespace() {
    ...
    for (ch, desc) in non_yaml_whitespace {
        ...
        println!("Non-YAML whitespace {} ({}): parse result = {:?}", desc, ch, result.is_ok());
        ...
        assert!(result_quoted.is_ok(), 
            "{} must work as content character in quotes per RFC 5.5", desc);
            
        if let Ok(docs) = result_quoted {
            if let Some(value) = docs[0]["key"].as_str() {
                assert!(value.contains(ch), 
                    "{} must be preserved as content, not treated as YAML whitespace", desc);
            }
        }
    }
}
...
"#;
    ...
    assert!(result.is_ok(), "Whitespace in mappings must work per RFC 5.5");
...
"#;
    ...
    assert!(result.is_ok(), "Whitespace in sequences must work per RFC 5.5");
    
    // Whitespace in flow collections
    let flow_whitespace = "[ item1 ,	item2 , 	item3 ]";
    let result = YamlLoader::load_from_str(flow_whitespace);
    assert!(result.is_ok(), "Whitespace in flow collections must work per RFC 5.5");
}
...
"#;
    ...
    assert!(result.is_ok(), "Quoted scalars with whitespace must work per RFC 5.5");
    
    if let Ok(docs) = result {
        if let Some(single_value) = docs[0]["single"].as_str() {
            assert!(single_value.contains("   "), "Multiple spaces must be preserved in single quotes");
            assert!(single_value.contains("\t"), "Tabs must be preserved in single quotes");
        }
        
        if let Some(double_value) = docs[0]["double"].as_str() {
            assert!(double_value.contains("   "), "Multiple spaces must be preserved in double quotes");
            assert!(double_value.contains("\t"), "Tabs must be preserved in double quotes");
        }
    }
}
...
"#;
    ...
    assert!(result.is_ok(), "Space-based indentation must work per RFC 5.5");
...
fn test_comprehensive_s_white_compliance() {
    ...
    assert!(result.is_ok(), "Comprehensive whitespace usage must work per RFC 5.5");
    
    if let Ok(docs) = result {
        // Verify all structures parsed correctly despite varied whitespace
        assert!(docs[0]["key1"].as_str().is_some(), "Space-separated mapping must work");
        assert!(docs[0]["key2"].as_str().is_some(), "Tab-separated mapping must work");
        assert!(docs[0]["flow"].as_vec().is_some(), "Flow with whitespace must work");
    }
}
...
fn test_whitespace_edge_cases() {
    ...
    assert!(result.is_ok(), "Leading/trailing whitespace must be handled per RFC 5.5");
    
    // Only whitespace content
    let whitespace_only = "key: '   \t   '";
    let result = YamlLoader::load_from_str(whitespace_only);
    assert!(result.is_ok(), "Whitespace-only quoted content must work per RFC 5.5");
