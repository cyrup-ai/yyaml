use yyaml::{YamlLoader, parse_str, Value};
...
fn test_yaml_1_2_bom_with_sequences() {
    ...
    let result: Vec<i32> = parse_str(yaml_with_bom).expect("BOM + sequence should parse per YAML 1.2");
...
"#;
    ...
    match &result {
        ...
        _ => {
            panic!("Result should be sequence per YAML 1.2 specification, got: {:?}", result);
        }
    }
}
