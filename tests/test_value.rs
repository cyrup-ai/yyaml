...
fn test_tagged() {
    ...
    let value = yyaml::to_value(&Enum::Variant(0)).unwrap();

    let deserialized: yyaml::Value = yyaml::from_value(value.clone()).unwrap();
    assert_eq!(value, deserialized);

    let serialized = yyaml::to_value(&value).unwrap();
